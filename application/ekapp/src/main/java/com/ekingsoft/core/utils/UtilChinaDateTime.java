 ...
 *
 */
public class UtilChinaDateTime {

    public static void main(String[] args) {
        UtilChinaDateTime l = new UtilChinaDateTime(System.currentTimeMillis());
    ...
    private synchronized void findFestival() {
        ...
        Matcher m;
        for (int i=0; i<UtilChinaDateTime.sFtv.length; i++) {
            m = UtilChinaDateTime.sFreg.matcher(UtilChinaDateTime.sFtv[i]);
            if (m.find()) {
                if (sM == UtilChinaDateTime.toInt(m.group(1)) && sD == UtilChinaDateTime.toInt(m.group(2))) {
        ...
        for (int i=0; i<UtilChinaDateTime.lFtv.length; i++) {
            m = UtilChinaDateTime.sFreg.matcher(UtilChinaDateTime.lFtv[i]);
            if (m.find()) {
                if (lM == UtilChinaDateTime.toInt(m.group(1)) && lD == UtilChinaDateTime.toInt(m.group(2))) {
        ...
        // 月周节日
        int w, d;
        for (int i=0; i<UtilChinaDateTime.wFtv.length; i++) {
            m = UtilChinaDateTime.wFreg.matcher(UtilChinaDateTime.wFtv[i]);
            if (m.find()) {
                if (this.getSolarMonth() == UtilChinaDateTime.toInt(m.group(1))) {
                    w = UtilChinaDateTime.toInt(m.group(2));
                    d = UtilChinaDateTime.toInt(m.group(3));
    ...
    private static int getLunarLeapMonth(int lunarYear) {
        ...
        int leapMonth = UtilChinaDateTime.lunarInfo[lunarYear - 1900] & 0xf;
    ...
    private static int getLunarLeapDays(int lunarYear) {
        ...
        // 若该年没有闰月,返回0
        return UtilChinaDateTime.getLunarLeapMonth(lunarYear) > 0 ? ((UtilChinaDateTime.lunarInfo[lunarYear - 1899] & 0xf) == 0xf ? 30
                : 29)
                : 0;
    }
    ...
    private static int getLunarYearDays(int lunarYear) {
        ...
        for (int i = 0x8000; i > 0x8; i >>= 1) {
            daysInLunarYear += ((UtilChinaDateTime.lunarInfo[lunarYear - 1900] & i) != 0) ? 1
                    : 0;
        }
        // 加上闰月天数
        daysInLunarYear += UtilChinaDateTime.getLunarLeapDays(lunarYear);
    ...
    private static int getLunarMonthDays(int lunarYear, int lunarMonth) {
        // 数据表中,每个农历年用16bit来表示,
        // 前12bit分别表示12个月份的大小月,最后4bit表示闰月
        int daysInLunarMonth = ((UtilChinaDateTime.lunarInfo[lunarYear - 1900] & (0x10000 >> lunarMonth)) != 0) ? 30
                : 29;
    ...
    public static synchronized int getUTCDay(Date date) {
            UtilChinaDateTime.makeUTCCalendar();
            synchronized (utcCal) {
                utcCal.clear();
    ...
    private static synchronized void makeUTCCalendar() {
        if (UtilChinaDateTime.utcCal == null) {
            UtilChinaDateTime.utcCal = new GregorianCalendar(TimeZone.getTimeZone("UTC"));
        }
    }
    /**
    ...
    private static int getSolarTermDay(int solarYear, int index) {
        ...
        l = l + UtilChinaDateTime.UTC(1900,0,6,2,5,0);
        return UtilChinaDateTime.getUTCDay(new Date(l));
    }
     ...
     * @param date 指定日期对象
     */
    public UtilChinaDateTime(Date date) {
        if (date == null)
            date = new Date();
     ...
     * @param TimeInMillis
     */
    public UtilChinaDateTime(long TimeInMillis) {
    ...
    private void init(long TimeInMillis) {
        ...
        this.lunarYear = 1900;
        int daysInLunarYear = UtilChinaDateTime.getLunarYearDays(this.lunarYear);
        while (this.lunarYear < 2100 && offset >= daysInLunarYear) {
            offset -= daysInLunarYear;
            daysInLunarYear = UtilChinaDateTime.getLunarYearDays(++this.lunarYear);
        }
        // 农历年数字
        ...
        int lunarMonth = 1;
        // 所在农历年闰哪个月,若没有返回0
        int leapMonth = UtilChinaDateTime.getLunarLeapMonth(this.lunarYear);
        // 是否闰年
        ...
        while (lunarMonth<13 && offset>0) {
            if (isLeap && leapDec) { // 如果是闰年,并且是闰月
                // 所在农历年闰月的天数
                daysInLunarMonth = UtilChinaDateTime.getLunarLeapDays(this.lunarYear);
                leapDec = false;
            } else {
                // 所在农历年指定月的天数
                daysInLunarMonth = UtilChinaDateTime.getLunarMonthDays(this.lunarYear, lunarMonth);
            }
    ...
    private void getCyclicalData() {
        ...
        int term2 = UtilChinaDateTime.getSolarTermDay(solarYear, 2); // 立春日期
        // 依节气调整二月分的年柱, 以立春为界
        ...
        int firstNode = UtilChinaDateTime.getSolarTermDay(solarYear, solarMonth * 2); // 传回当月「节」为几日开始
        // 依节气月柱, 以「节」为界
        ...
        cyclicalDay = (int) (UtilChinaDateTime.UTC(solarYear, solarMonth, solarDay, 0, 0, 0) / 86400000 + 25567 + 10) % 60;
    ...
    public String getAnimalString() {
        return UtilChinaDateTime.Animals[(this.lunarYear - 4) % 12];
    }
    ...
    public String getTermString() {
        // 二十四节气
        String termString = "";
        if (UtilChinaDateTime.getSolarTermDay(solarYear, solarMonth * 2) == solarDay) {
            termString = UtilChinaDateTime.solarTerm[solarMonth * 2];
        } else if (UtilChinaDateTime.getSolarTermDay(solarYear, solarMonth * 2 + 1) == solarDay) {
            termString = UtilChinaDateTime.solarTerm[solarMonth * 2 + 1];
        }
        return termString;
    }
    ...
    public int getTiananY() {
        return UtilChinaDateTime.getTianan(this.cyclicalYear);
    }
    ...
    public int getTiananM() {
        return UtilChinaDateTime.getTianan(this.cyclicalMonth);
    }
    ...
    public int getTiananD() {
        return UtilChinaDateTime.getTianan(this.cyclicalDay);
    }
     ...
     */
    public int getDeqiY() {
        return UtilChinaDateTime.getDeqi(this.cyclicalYear);
    }
     ...
     */
    public int getDeqiM() {
        return UtilChinaDateTime.getDeqi(this.cyclicalMonth);
    }
     ...
     */
    public int getDeqiD() {
        return UtilChinaDateTime.getDeqi(this.cyclicalDay);
    }
    ...
    public String getCyclicaYear() {
        return UtilChinaDateTime.getCyclicalString(this.cyclicalYear);
    }
    ...
    public String getCyclicaMonth() {
        return UtilChinaDateTime.getCyclicalString(this.cyclicalMonth);
    }
    ...
    public String getCyclicaDay() {
        return UtilChinaDateTime.getCyclicalString(this.cyclicalDay);
    }
    ...
    public String getLunarDayString() {
        return UtilChinaDateTime.getLunarDayString(this.lunarDay);
    }
    ...
    public String getLunarMonthString() {
        return (this.isLeap() ? "闰" : "") + UtilChinaDateTime.getLunarMonthString(this.lunarMonth);
    }
    ...
    public String getLunarYearString() {
        return UtilChinaDateTime.getLunarYearString(this.lunarYear);
    }
    ...
    private static String getCyclicalString(int cyclicalNumber) {
        return UtilChinaDateTime.Tianan[UtilChinaDateTime.getTianan(cyclicalNumber)] + UtilChinaDateTime.Deqi[UtilChinaDateTime.getDeqi(cyclicalNumber)];
    }
    ...
    private static String getLunarYearString(int lunarYear) {
        return UtilChinaDateTime.getCyclicalString(lunarYear - 1900 + 36);
    }
    ...
    private static String getLunarMonthString(int lunarMonth) {
        ...
        if (lunarMonth == 1) {
            lunarMonthString = UtilChinaDateTime.lunarString2[4];
        } else {
            if (lunarMonth > 9)
                lunarMonthString += UtilChinaDateTime.lunarString2[1];
            if (lunarMonth % 10 > 0)
                lunarMonthString += UtilChinaDateTime.lunarString1[lunarMonth % 10];
        }
    ...
    private static String getLunarDayString(int lunarDay) {
        ...
        int i1 = lunarDay / 10;
        int i2 = lunarDay % 10;
        String c1 = UtilChinaDateTime.lunarString2[i1];
        String c2 = UtilChinaDateTime.lunarString1[i2];
        if (lunarDay < 11) c1 = UtilChinaDateTime.lunarString2[0];
        if (i2 == 0) c2 = UtilChinaDateTime.lunarString2[1];
        return c1 + c2;
    }
}